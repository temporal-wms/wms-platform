...
```mermaid
graph TB
    subgraph "Packing Station"
        subgraph "Input"
            Totes[Incoming Totes]
        end
...
```mermaid
flowchart TD
    Start[Package Ready] --> Weight{Weight Check}

    Weight --> Zone{Delivery Zone}

    Zone --> Service{Service Level}

    Service -->|Standard| Standard[Ground Service]
    Service -->|Express| Express[2-Day Air]
    Service -->|Priority| Priority[Overnight]

    Standard --> Compare[Compare Carrier Rates]
    Express --> Compare
    Priority --> Compare

    Compare --> Select[Select Best Rate]
    Select --> Generate[Generate Label]
```
...
| PackagingSuggestedEvent | wms.packing.events | Package type selected |
| PackageSealedEvent | wms.packing.events | Package sealed |
| LabelAppliedEvent | wms.packing.events | Label affixed |
| PackTaskCompletedEvent | wms.packing.events | Task complete |

## Performance Metrics

| Metric | Description | Target |
|--------|-------------|--------|
| Pack Rate | Packages per hour | 30-50 packages/hr |
| Accuracy | Correct packages / Total | > 99.9% |
| Damage Rate | Damaged in transit | < 0.1% |
| Material Efficiency | Right-size packaging | > 95% |

## Related Diagrams

- [Picking Workflow](./picking-workflow) - Previous step (single item)
- [Shipping Workflow](./shipping-workflow) - Next step (SLAM)
