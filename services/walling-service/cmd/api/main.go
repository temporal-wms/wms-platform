...
import (
	...
	sharedMongo "github.com/wms-platform/shared/pkg/mongodb"
	"github.com/wms-platform/shared/pkg/tracing"
	httpHandlers "github.com/wms-platform/walling-service/internal/api/http"
...
const serviceName = "walling-service"
...
func main() {
	...
	logger.Info("Starting walling-service API")
	...
	mongoClient, err := sharedMongo.NewClient(ctx, config.MongoDB)
	if err != nil {
		logger.WithError(err).Error("Failed to connect to MongoDB")
		os.Exit(1)
	}
	instrumentedMongo := sharedMongo.NewInstrumentedClient(mongoClient, m, logger)
	...
	taskRepo := mongodb.NewWallingTaskRepository(instrumentedMongo.Database())
	...
	wallingService := application.NewWallingApplicationService(taskRepo, logger.Logger)
	...
	// Register API routes
	handlers := httpHandlers.NewHandlers(wallingService)
	httpHandlers.SetupRoutes(router, handlers)
...
func loadConfig() *Config {
	return &Config{
		...
		MongoDB: &sharedMongo.Config{
			...
			Database:       getEnv("MONGODB_DATABASE", "walling_db"),
