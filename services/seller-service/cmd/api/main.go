...
import (
	...
	"github.com/wms-platform/services/seller-service/internal/api/handlers"
	"github.com/wms-platform/services/seller-service/internal/application"
	mongoRepo "github.com/wms-platform/services/seller-service/internal/infrastructure/mongodb"
)

const serviceName = "seller-service"
...
func main() {
	...
	logger.Info("Starting seller-service API")
	...
	eventFactory := cloudevents.NewEventFactory("/seller-service")

	// Initialize repositories with instrumented client and event factory
	sellerRepo := mongoRepo.NewSellerRepository(instrumentedMongo.Database(), eventFactory)

	// Initialize and start outbox publisher
	outboxPublisher := outbox.NewPublisher(
		sellerRepo.GetOutboxRepository(),
	...
	sellerService := application.NewSellerApplicationService(
		sellerRepo,
		logger,
	)
	...
	v1.Use(middleware.RequireTenantAuth()) // All API routes require tenant headers
	{
		sellers := v1.Group("/sellers")
		{
			...
			sellers.GET("", sellerHandler.ListSellers)
			sellers.GET("/search", sellerHandler.SearchSellers)
			sellers.GET("/:sellerId", sellerHandler.GetSeller)

			// Status management
			sellers.PUT("/:sellerId/activate", sellerHandler.ActivateSeller)
			sellers.PUT("/:sellerId/suspend", sellerHandler.SuspendSeller)
			sellers.PUT("/:sellerId/close", sellerHandler.CloseSeller)

			// Facility management
			sellers.POST("/:sellerId/facilities", sellerHandler.AssignFacility)
			sellers.DELETE("/:sellerId/facilities/:facilityId", sellerHandler.RemoveFacility)

			// Fee schedule
			sellers.PUT("/:sellerId/fee-schedule", sellerHandler.UpdateFeeSchedule)

			// Channel integrations
			sellers.POST("/:sellerId/integrations", sellerHandler.ConnectChannel)
			sellers.DELETE("/:sellerId/integrations/:channelId", sellerHandler.DisconnectChannel)

			// API key management
			sellers.GET("/:sellerId/api-keys", sellerHandler.ListAPIKeys)
			sellers.POST("/:sellerId/api-keys", sellerHandler.GenerateAPIKey)
			sellers.DELETE("/:sellerId/api-keys/:keyId", sellerHandler.RevokeAPIKey)
		}
	}
...
func loadConfig() *Config {
	return &Config{
		...
		MongoDB: &mongodb.Config{
			...
			Database:       getEnv("MONGODB_DATABASE", "sellers_db"),
