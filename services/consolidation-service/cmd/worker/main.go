...
import (
	...
	"github.com/wms-platform/consolidation-service/internal/activities"
	mongoRepo "github.com/wms-platform/consolidation-service/internal/infrastructure/mongodb"
	"github.com/wms-platform/consolidation-service/internal/workflows"
...
func main() {
	...
	logger.Info("Starting consolidation-service worker")
	...
	repo := mongoRepo.NewConsolidationRepository(mongoClient.Database(), eventFactory)
	...
	// Create activities
	consolidationActivities := activities.NewConsolidationActivities(repo, logger)

	// Create worker
	workerOpts := temporal.DefaultWorkerOptions(temporal.TaskQueues.Consolidation)
	w := temporalClient.NewWorker(workerOpts)

	// Register workflow
	w.RegisterWorkflow(workflows.ConsolidationWorkflow)
	logger.Info("Registered workflow", "workflow", "ConsolidationWorkflow")

	// Register activities
	w.RegisterActivity(consolidationActivities.CreateConsolidationUnit)
	w.RegisterActivity(consolidationActivities.AssignStation)
	w.RegisterActivity(consolidationActivities.ConsolidateItem)
	w.RegisterActivity(consolidationActivities.CompleteConsolidation)
	w.RegisterActivity(consolidationActivities.MarkShort)
	...
	logger.Info("Worker started", "taskQueue", temporal.TaskQueues.Consolidation)
