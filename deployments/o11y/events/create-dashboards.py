#!/usr/bin/env python3
"""
Create Grafana Event-Driven dashboards for WMS Platform services.

Monitors Kafka event publishing and consuming metrics:
- Events Published Rate (by topic, event type)
- Events Consumed Rate (by topic, event type)
...
def build_panel(panel_template: dict, service: str, prometheus_uid: str) -> dict:
    ...
    # Skip datasource for row panels
    if panel_template["type"] != "row":
        panel["datasource"] = {"type": "prometheus", "uid": prometheus_uid}
        panel["targets"] = []
    ...
    # Add collapsed for row panels
    if "collapsed" in panel_template:
        panel["collapsed"] = panel_template["collapsed"]

    # Build targets (queries) - skip for row panels
    if panel_template["type"] != "row":
...
def create_dashboard_payload(service: str, config: dict) -> dict:
    ...
    return {
        "dashboard": {
            "title": f"{service}-events",
            "uid": f"{service}-events",
            "tags": ["wms", "kafka", "events", service],
...
def delete_existing_dashboards(grafana_url: str, auth: tuple) -> int:
    """Delete existing events dashboards."""
    # Search for dashboards with 'events' tag
    search_url = f"{grafana_url}/api/search?tag=events"
    credentials = base64.b64encode(f"{auth[0]}:{auth[1]}".encode()).decode()
    ...
    for dashboard in dashboards:
        if dashboard.get("title", "").endswith("-events"):
...
def main():
    parser = argparse.ArgumentParser(description="Create Grafana Event dashboards for WMS Platform")
    ...
    print("=" * 60)
    print("  WMS Platform - Event-Driven Dashboard Creator")
    print("=" * 60)
    ...
    for service in config["services"]:
        ...
        if result.get("status") == "success":
            print(f"  Created: {service}-events")
            success += 1
        else:
            error_msg = result.get('error', result.get('message', 'Unknown error'))
            print(f"  Failed: {service}-events - {error_msg}")
            failed += 1
    ...
    print("    - Total Events Published/Consumed")
    print("    - Publish Error Rate")
    print("    - P95 Publish Latency")
    print("    - Events Published Rate (by Topic)")
    print("    - Events Published Rate (by Event Type)")
    print("    - Publish Latency Distribution (p50, p95, p99)")
    print("    - Events Published by Status")
    print("    - Events Consumed Rate (by Topic)")
    print("    - Events Consumed Rate (by Event Type)")
    print("    - Consumer Lag")
    print("    - Events Consumed by Status")
    print("    - Event Types Breakdown (Pie Charts)")
    print()
