#!/usr/bin/env python3
"""
Create Grafana Failure Metrics dashboards for WMS Platform services.

Failure Metrics:
- Order failures by type and priority
- Retry attempts and outcomes
- Dead Letter Queue (DLQ) entries and resolutions
- Workflow failures by stage
- Reprocessing batch outcomes
...
def create_dashboard_payload(service: str, config: dict) -> dict:
    ...
    return {
        "dashboard": {
            "title": f"{service}-failure-metrics",
            "tags": ["wms", "failure-metrics", service],
...
def delete_existing_dashboards(grafana_url: str, auth: tuple) -> int:
    """Delete existing failure metrics dashboards."""
    # Search for dashboards with 'failure-metrics' tag
    search_url = f"{grafana_url}/api/search?tag=failure-metrics"
    credentials = base64.b64encode(f"{auth[0]}:{auth[1]}".encode()).decode()
    ...
    for dashboard in dashboards:
        if dashboard.get("title", "").endswith("-failure-metrics"):
...
def main():
    parser = argparse.ArgumentParser(description="Create Grafana Failure Metrics dashboards for WMS Platform")
    ...
    print("=" * 60)
    print("WMS Platform - Failure Metrics Dashboard Creator")
    print("=" * 60)
    ...
    for service in config["services"]:
        ...
        if result.get("status") == "success":
            print(f"  Created: {service}-failure-metrics")
            success += 1
        else:
            error_msg = result.get('error', result.get('message', 'Unknown error'))
            print(f"  Failed: {service}-failure-metrics - {error_msg}")
            failed += 1
    ...
    print("  - Order Failures by Type")
    print("  - Retry Outcomes (success/failure/dlq)")
    print("  - Retry Success Rate (%)")
    print("  - DLQ Entries Pending")
    print("  - DLQ Entries by Failure Type")
    print("  - Retry Attempts by Failure Type")
    print("  - Workflow Failures by Stage")
    print("  - Workflow Retries")
    print("  - DLQ Resolutions by Type")
    print("  - DLQ Age Distribution")
    print("  - Reprocessing Batch Outcomes")
    print("  - Failures by Priority (table)")
    print("  - Retry Duration Percentiles")
    print()
